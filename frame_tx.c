...
static void frame_notify_received(const unsigned char* frame_buf, int len) {
    ...
    /* frame[] should contain clean data with checksum but without header and trailer */
    unsigned short my_cksum = crc_xmodem(frame, frame_pos - 2);
    unsigned short msg_cksum = *(unsigned short*)(frame + frame_pos - 2); // Same, little endian
...
void frame_notify_serial_recv() {
    ...
    int recv_len = serial_read(g_serial_recv_buf + g_recv_buf_pos, sizeof(g_serial_recv_buf) - g_recv_buf_pos);
    ...
    while (remaining_search_range > 0 &&
            (last_trailer = memchr(search_pos, FRAME_TRAILER, remaining_search_range)) != NULL) {
        if (*(last_trailer - 1) != FRAME_ESCAPE) {
            /* Not escaped. This is the end of the frame */
            frame_notify_received(g_serial_recv_buf, last_trailer - g_serial_recv_buf + 1);
            g_recv_buf_pos = 0;
            break; /* Do not support continous frames */
        } else {
            /* Escaped. Continue search from this place */
            search_pos = last_trailer + 1;
            remaining_search_range = g_recv_buf_pos - (search_pos - g_serial_recv_buf);
        }
    }
}
